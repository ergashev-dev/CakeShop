    ...
    <div className="flex min-h-screen bg-[#F7F8FA] dark:bg-[#0F1012] text-[#17181A] dark:text-[#F3F4F6] font-sans antialiased">
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-y-auto">
        ...
        <div className="p-3 sm:p-6 lg:p-8 space-y-6">
          ...
          {/* 12. ALL ORDERS MANAGEMENT */}
          {activeTab === 'orders' && isAdminRole && (
            <div className="space-y-6 animate-in fade-in duration-200">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                  <h3 className="text-lg font-bold">Barcha Buyurtmalar Boshqaruvi</h3>
                  <p className="text-xs text-[#6B7280]">
                    Mijozlarning barcha buyurtmalarini kuzatish, qidirish va holatini boshqarish.
                  </p>
                </div>
                <div className="flex items-center gap-2">
                  <span className="text-xs font-semibold text-[#6B7280]">
                    Jami: <strong className="text-[#17181A] dark:text-[#F3F4F6]">{orders.length}</strong> ta buyurtma
                  </span>
                </div>
              </div>

              {/* Search & Status Filters */}
              <div className="bg-white dark:bg-[#16181D] border border-[#E7E9ED] dark:border-[#272A30] rounded-2xl p-4 shadow-card space-y-3">
                <div className="flex flex-col md:flex-row gap-3">
                  <div className="relative flex-1">
                    <Search className="w-4 h-4 text-[#9CA3AF] absolute left-3.5 top-1/2 -translate-y-1/2" />
                    <input
                      type="text"
                      placeholder="Buyurtma ID, mijoz ismi, telefon yoki manzil bo‘yicha qidiruv..."
                      value={orderSearchQuery}
                      onChange={(e) => setOrderSearchQuery(e.target.value)}
                      className="w-full pl-10 pr-4 py-2 rounded-xl border border-[#E7E9ED] dark:border-[#272A30] bg-[#F7F8FA] dark:bg-[#1F2227] text-xs outline-none focus:ring-2 focus:ring-[#2563EB]/20"
                    />
                  </div>
                </div>
                ...
                <div className="flex items-center gap-1.5 overflow-x-auto no-scrollbar pt-1">
                  {[
                    ...
                    { id: 'preparing', label: 'Pishirilmoqda' },
                    ...
                    <button
                      ...
                      className={`px-3 py-1.5 rounded-lg text-xs font-semibold whitespace-nowrap transition-colors cursor-pointer ${
                        orderStatusFilter === tab.id
                          ...
                          : 'bg-[#F7F8FA] dark:bg-[#202328] text-[#4B5563] dark:text-[#9CA3AF] hover:bg-[#E7E9ED] dark:hover:bg-[#2A2D35]'
                      }`}
                    >
                      {tab.label}
                      {tab.id !== 'all' && (
                  ...
                  {filteredOrders.map((ord) => (
                    ...
                    >
                      <div className="flex items-center justify-between gap-2">
                        ...
                        <div className="flex items-center gap-1.5">
                          ...
                          <button
                            ...
                            className="p-1.5 rounded-lg border border-[#E7E9ED] dark:border-[#272A30] hover:bg-[#F7F8FA] dark:hover:bg-[#202328] text-[#2563EB] cursor-pointer transition-colors"
                            title="Tafsilotlar"
                          >
                      ...
                      <div className="text-xs space-y-1">
                        <div className="flex items-center justify-between">
                          <span className="font-semibold text-[#111827] dark:text-[#F3F4F6]">{ord.customer_name}</span>
                          <span className="font-black text-sm text-[#111827] dark:text-[#F3F4F6]">{formatPrice(ord.total)}</span>
                        </div>
                        <div className="flex items-center justify-between text-[11px] text-[#6B7280]">
                          <span>{ord.customer_phone}</span>
                          <span className="uppercase font-bold text-[10px] px-2 py-0.5 rounded bg-gray-100 dark:bg-[#202328] text-[#4B5563] dark:text-[#9CA3AF]">{ord.payment_method}</span>
                        </div>
                        {ord.customer_address && (
                          <div className="text-[11px] text-[#6B7280] truncate pt-0.5">
                            📍 {ord.customer_address}
                          </div>
                        )}
                        <div className="text-[11px] text-[#4B5563] dark:text-[#9CA3AF] pt-1.5 border-t border-[#E7E9ED]/50 dark:border-[#272A30]/50 truncate">
                          📦 {ord.items?.length || 0} ta mahsulot: {ord.items?.map(i => i.name).slice(0, 2).join(', ')}
                        </div>
                      </div>
                ...
                <div className="hidden sm:block overflow-x-auto">
                  <table className="w-full text-left text-xs min-w-[850px]">
                    <thead>
                      <tr className="border-b border-[#E7E9ED] dark:border-[#272A30] text-[#6B7280] bg-[#F9FAFB] dark:bg-[#1A1C22]">
                        <th className="px-4 py-3 font-semibold whitespace-nowrap">ID</th>
                        <th className="px-4 py-3 font-semibold whitespace-nowrap">Sana</th>
                        <th className="px-4 py-3 font-semibold whitespace-nowrap">Mijoz</th>
                        <th className="px-4 py-3 font-semibold whitespace-nowrap">Telefon</th>
                    ...
                    <tbody className="divide-y divide-[#E7E9ED]/50 dark:divide-[#272A30]/50">
                      {filteredOrders.map((ord) => (
                        <tr key={ord._id} className="hover:bg-[#F7F8FA] dark:hover:bg-[#1E2026] transition-colors">
                          ...
                          <td className="px-4 py-3.5 font-semibold max-w-[140px]">
                            <div className="truncate" title={ord.customer_name}>{ord.customer_name}</div>
                            {ord.user?.username && (
                              <span className="text-[10px] text-[#2563EB] block truncate">@{ord.user.username}</span>
                            )}
                          </td>
                          <td className="px-4 py-3.5 text-[#6B7280] whitespace-nowrap">{ord.customer_phone}</td>
                          <td className="px-4 py-3.5 text-[11px] text-[#4B5563] dark:text-[#9CA3AF] max-w-[150px] truncate" title={ord.customer_address}>
                            {ord.customer_address || '-'}
                          </td>
                          <td className="px-4 py-3.5">
                            <span className="font-semibold text-xs">{ord.items?.length || 0} ta</span>
                            <span className="block text-[10px] text-[#6B7280] truncate max-w-[120px]">
                              {ord.items?.[0]?.name}
                            </span>
                          </td>
                          <td className="px-4 py-3.5 font-black text-[#17181A] dark:text-[#F3F4F6] whitespace-nowrap">
                            {formatPrice(ord.total)}
                          </td>
                          <td className="px-4 py-3.5 uppercase font-bold text-[10px] whitespace-nowrap">{ord.payment_method}</td>
                          <td className="px-4 py-3.5 whitespace-nowrap">{getStatusBadge(ord.status)}</td>
                          <td className="px-4 py-3.5 text-right whitespace-nowrap">
                            <div className="flex items-center justify-end gap-1.5">
                              ...
                              <button
                                onClick={() => setSelectedOrderDetails(ord)}
                                className="p-1.5 rounded-lg border border-[#E7E9ED] dark:border-[#272A30] hover:bg-[#F7F8FA] dark:hover:bg-[#202328] text-[#2563EB] cursor-pointer"
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
                  {filteredCakes.map((cake) => (
                    ...
                    >
                      ...
                      <div className="pt-3 mt-3 border-t border-[#E7E9ED] dark:border-[#272A30] flex items-center justify-between">
                        <div>
                        ...
                        <button
                          ...
                          className="p-2 rounded-xl text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors cursor-pointer"
                          title="O‘chirish"
                        >
                          <Trash2 className="w-4 h-4" />
                        </button>
              ...
              <div className="space-y-1.5">
                ...
                <CustomSelect
                  ...
                  options={[
                    ...
                    { value: 'confirmed', label: 'Tasdiqlandi (confirmed)' },
                    { value: 'preparing', label: 'Pishirilmoqda (preparing)' },
                    { value: 'ready', label: 'Tayyor (ready)' },
                    { value: 'delivering', label: 'Yetkazilmoqda (delivering)' },
                    { value: 'delivered', label: 'Yetkazildi (delivered)' },
                    { value: 'cancelled', label: 'Bekor qilindi (cancelled)' },
                  ]}
                />
              </div>
            </div>
